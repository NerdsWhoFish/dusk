...
- [~] **A view can mount on the plugin's own page**: a contribution names its slot, so something that is about the plugin rather than about one entity has somewhere to go. Creating a thing the catalog has never seen has no entity to mount against. **Only a drawn element works there.** `Contributions` renders `PluginBlock` with no entities, so a *declared* spec in the plugin slot always falls through to its `empty` text. An element fetches its own data and does not notice; a spec has nothing to render and cannot say so. Either the plugin page resolves entities for the slot the way a homepage `view` block resolves a query ([0035](../adr/0035-blocks-resolve-server-side.md)), or `conformance.ValidateDescribe` rejects a spec in that slot so it fails loudly in the plugin instead of silently in Dusk
...
- [x] **A plugin can ask the human a question**: an action whose input is a judgement rather than a lookup can insist a person supplied it, instead of trusting that the calling agent did not invent it. The question is **returned, not called**: `InvokeResponse.elicit` ends the turn and Dusk invokes again with the answer, which is what lets a surface with nobody attached reply `unsupported` rather than hanging. That keeps one declaration working from an agent, the UI and a schedule ([0041](../adr/0041-plugins-reach-agents-as-actions.md)), and keeps `Invoke` unary so [0039](../adr/0039-one-plugin-transport.md) is not re-decided by making the contract's central RPC streaming. Additive, so the plugins written before it are untouched. Bounded at four turns, because a question is a loop waiting to happen. **The UI has no elicitor yet**, so a plugin that asks behaves differently in the browser than over MCP ([0046](../adr/0046-plugins-can-ask.md))
...
- [~] **Flux**: what GitOps believes is deployed, against what Kubernetes reports running, in `dusk-plugin-flux`. Written and committed, not released, and verified read only against two real clusters. Nine kinds resolved by discovery rather than compiled in versions, because the typed Flux API pulls 161 modules against 68 for the dynamic client to read about fifteen field paths, and `ImagePolicy` moved its result field between versions anyway. A release links to the services it deployed using the Kubernetes plugin's own refs, taken from `status.inventory` rather than guessed, which is the first time two plugins meet on one entity. An absent CRD is not a partial view: one cluster runs no image automation, and calling that partial would mean it never ingests at all. The suspended versus broken distinction is modelled and unit tested rather than observed, since nothing on either cluster is currently suspended or failing
- [~] **OCI registries**: Harbor, GHCR and friends, in `dusk-plugin-oci`. Written and committed, not released, and verified against three real registries. The Distribution API rather than any vendor's, so one plugin serves all of them. A repository is the entity and **a tag is not**: one public image carries 1385 tags, and an entity each would make a registry the largest source in any estate. Which tags ride along is decided from the tag list alone, never from the registry's ordering, which is not portable between them.
...
4. Pinned notes reaching agent context. The proto documents `pinned` as "candidates for injection into agent context at boot" and `ContextBudget` reasons explicitly about what pinning costs, but `dusk_context` renders owned entities, inventory and a drift summary, and no notes at all. Pinning currently only affects list ordering. This is the shortest path from "the operator wrote down a gotcha" to "the agent knew it before it started", and the budget arithmetic it needs is the only real work.
5. A plugin that dies stays dead. Nothing restarts one, so a crash is an outage until somebody notices, and it is now an outage of something that could be acting rather than only observing.
...
- Nothing restarts a plugin whose process died. Every call to it fails until Dusk restarts, and that now takes actions down as well as observation.
