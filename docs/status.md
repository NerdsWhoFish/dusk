...
- [ ] **Container hosts**: everything running outside Kubernetes, which is currently everything Dusk cannot see at all. A NAS, a Docker box, a machine running one service natively. The Music Assistant plugin already catalogs players while the server driving them is uncatalogued, which is the hole visible from inside the catalog. Open question in its own repo: whether this speaks plain Docker with Synology as a dialect, or is a Synology plugin that stays honest about its scope
