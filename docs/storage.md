...
The index is the materialized entity graph: SQLite, one file, keyed by git ref.
...
A **git ref** is a branch or pull request ref such as `refs/heads/main`, and it is the key the whole index is partitioned by.
...
| Table | What |
| --- | --- |
| `entities` | One row per entity per git ref. Primary key is `(git_ref, ref)` |
| `relations` | Typed edges. Primary key is `(git_ref, from_ref, to_ref, type)` |
...
| Call | What it does |
| --- | --- |
| `Put` | Replaces everything at a git ref, in one transaction |
...
| `DropGitRef` | Removes everything at a git ref |
...
Searches are scoped to one git ref, like everything else.
