...
| `embedding` | Calling an OpenAI-compatible embeddings endpoint and validating one finite vector per input | Storing vectors, ranking search, or choosing a model. Those are deployment and index policy ([ADR-0083](../adr/0083-search-fuses-exact-full-text-and-semantic-retrieval.md)) |
...
| `events` | The durable record of what an action invocation did, kept in a bounded buffer beside the disposable index and also logged | Becoming an unbounded audit service. Its history exists for recent operations and retry receipts, not compliance ([ADR-0015](../adr/0015-plugin-actions-and-events.md), [ADR-0070](../adr/0070-mutations-have-durable-retry-identity.md)) |
...
| `insights` | Deriving operator-facing analytics from the current local catalog and bounded action history | Tracking browser behavior, storing analytics events, reaching GitHub, or exporting data ([ADR-0088](../adr/0088-dashboard-analytics-are-derived-locally.md)) |
