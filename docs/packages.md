...
| `telemetry` | The process-wide OpenTelemetry provider, HTTP instrumentation, propagation, and trace correlation for structured logs | Choosing the backend, holding its credentials, or becoming a shared instrumentation library for other repositories ([0078](../adr/0078-runtime-telemetry-uses-explicit-opentelemetry-sdks.md)) |
