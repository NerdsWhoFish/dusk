...
**One operator view.** A bearer token answers "may you read" and grants the whole catalog. GitHub login admits an identity through its overlap with the installed App, then grants that same complete operator view. `mcp.Server.viewer` therefore answers `Unrestricted`, matching the browser ([ADR-0084](../adr/0084-github-login-grants-the-complete-operator-view.md)).
