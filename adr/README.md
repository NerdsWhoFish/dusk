...
| [0048](0048-the-kind-vocabulary.md) | Kinds are derived, minted with a role, and never rejected | Fills in what 0010 sketched as `getKinds` and `mintKind`. The vocabulary is counted out of the index so it cannot go stale, and a mint adds only what derivation cannot reach: a role and aliases. Every role has a consequence, so nothing decorative can be minted; `reference` gives 0045 the precision it was blocked on. A near match warns and never refuses, because rejecting would delete 0007's open vocabulary. Also names the reserved paths in `.dusk/`, which a reconcile was failing on |
