...
func note(id, kind string, refs ...string) *duskv1alpha1.Note {
...
func TestADR0049_AGotchaOutranksATodoWithoutBeingPinned(t *testing.T) {
	...
	err := db.Put(t.Context(), testRepo, mainRef,
		...
		[]*duskv1alpha1.Note{
			note(".dusk/a-todo.md", "todo", ref),
			note(".dusk/b-runbook.md", "runbook", ref),
			note(".dusk/c-gotcha.md", "gotcha", ref),
		})
	if err != nil {
		t.Fatalf("Put: %v", err)
	}
...
func TestPinningStillOutranksTheKind(t *testing.T) {
	...
	pinned := note(".dusk/z-todo.md", "todo", ref)
	pinned.Pinned = true
	err := db.Put(t.Context(), testRepo, mainRef,
		declare([]*duskv1alpha1.Entity{entity(ref, "Jellyfin", "")}), nil,
		[]*duskv1alpha1.Note{note(".dusk/a-gotcha.md", "gotcha", ref), pinned})
	if err != nil {
		t.Fatalf("Put: %v", err)
	}
...
func TestAMintedNoteKindRanks(t *testing.T) {
	...
	err := db.Put(t.Context(), testRepo, mainRef,
		declare([]*duskv1alpha1.Entity{entity(ref, "Jellyfin", "")}), nil,
		[]*duskv1alpha1.Note{
			note(".dusk/a-runbook.md", "runbook", ref),
			note(".dusk/b-postmortem.md", "postmortem", ref),
		})
	if err != nil {
		t.Fatalf("Put: %v", err)
	}
