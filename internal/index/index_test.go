...
func TestDefaultViewSpansEachRepositorysOwnBranch(t *testing.T) {
	...
	t.Run("search spans them too", func(t *testing.T) {
		results, err := db.Search(ctx, "", index.SearchFilter{Query: "media", Limit: 10})
		if err != nil {
...
func TestSearch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			results, err := db.Search(ctx, mainRef, index.SearchFilter{Query: tt.query, Limit: 10})
			if err != nil {
	...
	t.Run("a hit carries a snippet of the match", func(t *testing.T) {
		results, err := db.Search(ctx, mainRef, index.SearchFilter{Query: "transcoding", Limit: 10})
		if err != nil {
			t.Fatalf("Search: %v", err)
		}
	...
	t.Run("punctuation cannot produce a query syntax error", func(t *testing.T) {
		for _, query := range []string{`"unbalanced`, "kind:service", "a AND (b", "*", `foo"bar`} {
			if _, err := db.Search(ctx, mainRef, index.SearchFilter{Query: query, Limit: 10}); err != nil {
	...
	t.Run("an empty query is rejected", func(t *testing.T) {
		if _, err := db.Search(ctx, mainRef, index.SearchFilter{Query: "   ", Limit: 10}); err == nil {
...
func TestSearchIsScopedToOneGitRef(t *testing.T) {
	...
	results, err := db.Search(ctx, mainRef, index.SearchFilter{Query: "navidrome", Limit: 10})
	if err != nil {
		t.Fatalf("Search: %v", err)
	}
	if len(results) != 0 {
