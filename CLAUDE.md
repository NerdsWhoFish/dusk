...
- **Everything here releases through [Quill](https://github.com/TheOutdoorProgrammer/quill)**, on the same dispatch-with-scope-and-bump flow ([ADR-0021](adr/0021-release-tooling.md)). What differs is the publisher it is pointed at: a plugin says `publish: goreleaser`, because it ships per-platform binaries Dusk downloads from a GitHub release, and Dusk says `publish: docker`, because it ships a container image and no binaries. Dusk still does **not** use GoReleaser, for that reason, and a plugin still does. Do not "fix" either to match the other; point quill at the right publisher.
